...
def apply_ui_to_settings(settings) -> None:
    """Mutate Settings with UI overlay (scan + credentials)."""
    ui = load_ui_settings()
    ...
    settings.max_alerts_per_tier = int(ui["max_alerts_per_tier"])  # type: ignore[attr-defined]
    settings.enrich_on_alert = bool(ui["enrich_on_alert"])  # type: ignore[attr-defined]
    settings.no_repeat_same_day = bool(ui["no_repeat_same_day"])  # type: ignore[attr-defined]
    settings.allow_cheaper_repeat = bool(ui["allow_cheaper_repeat"])  # type: ignore[attr-defined]
    ...
    settings.channel_enabled = {  # type: ignore[attr-defined]
        t: bool(ui.get(f"channel_enabled_{t}", True)) for t in (60, 70, 80, 90)
    }
